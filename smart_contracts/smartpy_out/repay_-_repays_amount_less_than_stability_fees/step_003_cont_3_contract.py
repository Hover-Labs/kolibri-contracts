...
class Contract(sp.Contract):
  def __init__(self):
    self.init(administratorContractAddress = sp.address('tz1VmiY38m3y95HqQLjMwqnMS7sdMfGomzKi'), governorContractAddress = sp.address('tz1abmz7jiCV2GH2u81LRrGgAFFgvQgiDiaf'), ovenRegistryContractAddress = sp.address('tz1hAYfexyzPGG6RhZZMpDvAHifubsbb6kgn'), savingsAccountContractAddress = sp.address('tz1gfArv665EUkSg2ojMBzcbfwuPxAvqPvjo'), tokenContractAddress = sp.address('tz1cYufsxHXJcvANhvS55h3aY32a9BAFB494'))

  @sp.entry_point
  def accrueInterest(self, params):
    sp.set_type(params, sp.TNat)
    sp.verify(sp.sender == self.data.savingsAccountContractAddress, message = 27)
    sp.transfer(sp.record(from_ = sp.self_address, to_ = self.data.savingsAccountContractAddress, value = params), sp.tez(0), sp.contract(sp.TRecord(from_ = sp.TAddress, to_ = sp.TAddress, value = sp.TNat).layout(("from_ as from", ("to_ as to", "value"))), self.data.tokenContractAddress, entry_point='transfer').open_some())
  ...
  @sp.entry_point
  def setSavingsAccountContract(self, params):
    sp.set_type(params, sp.TAddress)
    sp.verify(sp.sender == self.data.governorContractAddress, message = 4)
    self.data.savingsAccountContractAddress = params