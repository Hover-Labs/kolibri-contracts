...
class Contract(sp.Contract):
  def __init__(self):
    self.init_type(sp.TRecord(borrowedTokens = sp.TNat, interestIndex = sp.TInt, isLiquidated = sp.TBool, ovenProxyContractAddress = sp.TAddress, owner = sp.TAddress, stabilityFeeTokens = sp.TInt).layout((("borrowedTokens", ("interestIndex", "isLiquidated")), ("ovenProxyContractAddress", ("owner", "stabilityFeeTokens")))))
    self.init(borrowedTokens = 0,
              interestIndex = 1000000000000000000,
              isLiquidated = False,
              ovenProxyContractAddress = sp.address('KT1Tezooo6zzSmartPyzzSTATiCzzztY1196'),
              owner = sp.address('KT1Tezooo9zzSmartPyzzSTATiCzzztdBMLX'),
              stabilityFeeTokens = 0)

  @sp.entry_point
  def borrow(self, params):
    sp.set_type(params, sp.TNat)
    sp.verify(sp.sender == self.data.owner, 6)
  ...
  def updateState(self, params):
    ...
    sp.verify(sp.fst(params) == sp.self_address, 13)
