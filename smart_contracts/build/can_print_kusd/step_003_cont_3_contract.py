...
class Contract(sp.Contract):
  def __init__(self):
    self.init_type(sp.TRecord(clientCallback = sp.TOption(sp.TAddress), governorContractAddress = sp.TAddress, harbingerContractAddress = sp.TAddress, maxDataDelaySec = sp.TNat, state = sp.TIntOrNat).layout((("clientCallback", "governorContractAddress"), ("harbingerContractAddress", ("maxDataDelaySec", "state")))))
    self.init(clientCallback = sp.none,
              governorContractAddress = sp.address('tz1abmz7jiCV2GH2u81LRrGgAFFgvQgiDiaf'),
              harbingerContractAddress = sp.address('KT1Tezooo2zzSmartPyzzSTATiCzzzwqqQ4H'),
              maxDataDelaySec = 1800,
              state = 0)
  ...
  def getXtzUsdRate_callback(self, params):
    ...
    sp.verify(sp.sender == self.data.harbingerContractAddress, 3)
    sp.verify(sp.fst(params) == 'XTZ-USD', 14)
    sp.verify(sp.as_nat(sp.now - sp.fst(sp.snd(params))) < self.data.maxDataDelaySec, 17)
