...
class MinterContract(sp.Contract):
    ...
    @sp.entry_point
    def setAmountLoaned(self, newAmountLoaned):
        sp.verify(sp.sender == self.data.governorContractAddress, message = Errors.NOT_GOVERNOR)
        self.data.amountLoaned = newAmountLoaned              
...
if __name__ == "__main__":
    ...
    class LambdaTester(sp.Contract):
        ...
        def compute(self, param):
            self.data.result = sp.some(self.f(param))
    ...
    def test():
        ...
        tester = LambdaTester(minter.calculateNewAccruedInterest)
        scenario += tester

        scenario += tester.compute(
            (sp.to_int(1 * Constants.PRECISION), (100 * Constants.PRECISION, (0 * Constants.PRECISION,  1100000000000000000)))
        )
        scenario.verify(tester.data.result.open_some() == 10 * Constants.PRECISION)

        scenario += tester.compute(
            (sp.int(1100000000000000000), (100 * Constants.PRECISION, (10 * Constants.PRECISION, 1210000000000000000)))
        )
        scenario.verify(tester.data.result.open_some() == 11 * Constants.PRECISION)

        scenario += tester.compute(
            (sp.to_int(1 * Constants.PRECISION), (100 * Constants.PRECISION, (0 * Constants.PRECISION,  1210000000000000000)))
        )
        scenario.verify(tester.data.result.open_some() == 21 * Constants.PRECISION)

        scenario += tester.compute(
            (sp.int(1100000000000000000), (100 * Constants.PRECISION, (0 * Constants.PRECISION,  1210000000000000000)))
        )
        scenario.verify(tester.data.result.open_some() == 10 * Constants.PRECISION)

        scenario += tester.compute(
            (sp.int(1210000000000000000), (100 * Constants.PRECISION, (10 * Constants.PRECISION, 1331000000000000000)))
        )
        scenario.verify(tester.data.result.open_some() == 11 * Constants.PRECISION)

        scenario += tester.compute(
            (sp.int(1100000000000000000), (100 * Constants.PRECISION, (0 * Constants.PRECISION,  1331000000000000000)))
        )
        scenario.verify(tester.data.result.open_some() == 21 * Constants.PRECISION)        
    ...
    def test():
        ...
        tester = UnitViewTester(minter.getCurrentInterestIndex)
        scenario += tester
    ...
    def test():
        ...
        # AND a tester to test the view at different points in time
        tester = UnitViewTester(minter.getAmountLoaned)
        scenario += tester
        ...
        scenario.verify(tester.data.result.open_some() == amountLoaned)
        ...
        # THEN the original amountLoaded is linearly approximated for one period
        scenario.verify(tester.data.result == sp.some(1100000000000000000))
    ...
    def test():
        ...
        # THEN the original amountLoaned is returned
        scenario.verify(tester.data.result.open_some() == 10 * Constants.PRECISION) # 10 kUSD - the original amount
        ...
        # THEN the original amountLoaned is returned
        scenario.verify(tester.data.result.open_some() == 11 * Constants.PRECISION) # 11 kUSD - 10 kUSD linearly approximated for one period
        ...
        # THEN the original amountLoaned is returned
        scenario.verify(tester.data.result.open_some() == 12 * Constants.PRECISION) # 12 kUSD - 10 kUSD linearly approximated for two periods
    ...
    @sp.add_test(name="setAmountLoaned - can reset amount loaned")
    def test():
        scenario = sp.test_scenario()

        # GIVEN a minter contract with an amount loaned
        governor = Addresses.GOVERNOR_ADDRESS
        amountLoaned = 10 * Constants.PRECISION # 10 kUSD
        minter = MinterContract(
            amountLoaned = amountLoaned,
            governorContractAddress = governor
        )
        ...
        # WHEN setAmountLoaned is called
        newAmountLoaned = 200 * Constants.PRECISION # 200 kUSD
        scenario += minter.setAmountLoaned(newAmountLoaned).run(
            sender = governor
        )

        # THEN the amountLoaned is updated
        scenario.verify(minter.data.amountLoaned == newAmountLoaned)

    @sp.add_test(name="setAmountLoaned - fails if not called by governor")
    def test():
        scenario = sp.test_scenario()
        ...
        # WHEN setAmountLoaned is called by someone other than the governor
        # THEN the call fails with NOT_GOVERNOR
        notGovernor = Addresses.NULL_ADDRESS
        newAmountLoaned = 200 * Constants.PRECISION # 200 kUSD
        scenario += minter.setAmountLoaned(newAmountLoaned).run(
